...
def draw_graph_2(nome_giornali_data_all: list, min_value: int, max_value: int, j, HOURS, couples):
    ...
    for i in range(0, len(nome_giornali_data_all)):
        ...
        plt.savefig(f"BINARY_ANALISI_ORARIA_{name}_{j - no_graph + 1}_{j}.png", dpi=100)
        # plt.show(block=True)
        plt.close()
...
def draw_ratio(nome_giornali_data_all: list, min_value: int, max_value: int, j, HOURS, couples):
    ...
    for i in range(0, len(nome_giornali_data_all)):
        ...
        name = "_".join(couples).replace("/", "_")
        plt.savefig(f"BINARY_RATIO_ANALISI_ORARIA_{name}_{j - no_graph + 1}_{j}.png", dpi=100)
        # plt.show(block=True)
        plt.close()
...
def main():
    ...
    for (index_a, index_b) in couples:
        ...
        for i in range(0, 24):
            ...
            if i % 4 == 3:
                draw_graph_2(nome_giornali_local_time[i - 3: i + 1], min_value, max_value, i, HOURS, COUPLE_PAPERS)
                draw_ratio(nome_giornali_local_time[i - 3: i + 1], min_value, max_value, i, HOURS, COUPLE_PAPERS)
