...
def draw_graph_2(nome_giornali_data_all: list, min_value: int, max_value: int, j, HOURS, DAYS, theme):
    ...
    plt.savefig(f"TEMI_4D_{theme}_{DAYS[0]}_{DAYS[3]}.png", dpi=100)
    # plt.show(block=True)
    plt.close()

def draw_ratio(nome_giornali_data_all: list, min_value: int, max_value: int, j, HOURS, DAYS, theme):
    fig, axs = plt.subplots(2, 2, figsize=(25, 15))
    for LINE in range(0, 2):
        for COL in range(0, 2):
            index_hr = LINE * 2 + COL
    ...
    plt.savefig(f"RATIO_TEMI_4D_{theme}_{DAYS[0]}_{DAYS[3]}.png", dpi=100)
    # plt.show(block=True)
    plt.close()
...
def main():
    ...
    for theme in themes:
            ...
            for (label, path) in enumerate(NEWS_PAPERS):
                ...
                for rp in path:
                    extracted_articles.extend(
                        extract_articles(BASE_DIR, rp, DAY, f"00:00:00", f"23:59:59"))
                extracted_articles = extract_theme_articles(extracted_articles, theme)
                extracted_articles_st = []
                for rp in path:
                    extracted_articles_st.extend(extract_articles_scraping_time(BASE_DIR, rp, DAY, f"00:00:00",
                                                                                f"23:59:59"))
        ...
        draw_ratio(nome_giornali_local_time, 0, 0, 0, 24, DAYS, theme)
        draw_graph_2(nome_giornali_local_time, 0, 0, 0, 24, DAYS, theme)
