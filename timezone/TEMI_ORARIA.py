...
from utils import calculate_similarity, extract_articles, array_nlp_cache, \
...
def draw_ratio(nome_giornali_data_all: list, min_value: int, max_value: int, j, HOURS, theme):
    ...
    for i in range(0, len(nome_giornali_data_all)):
        ...
        plt.savefig(f"RATIO_TEMI_ORARIA_{theme}_{j - no_graph + 1}_{j}.png", dpi=100)
        # plt.show(block=True)
        plt.close()
...
def main():
    ...
    for theme in themes:
        for i in range(0, 24):
            ...
            if i % 4 == 3:
                draw_ratio(nome_giornali_local_time[i - 3: i + 1], min_value, max_value, i, HOURS, theme)
                draw_graph_2(nome_giornali_local_time[i - 3: i + 1], min_value, max_value, i, HOURS, theme)
