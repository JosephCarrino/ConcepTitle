...
def draw_graph_2(nome_giornali_data_all: list, min_value: int, max_value: int, j, HOURS, DAYS):
    ...
    plt.savefig(f"ANALISI_4D_{DAYS[0]}_{DAYS[3]}.png", dpi=100)
    # plt.show(block=True)
    plt.close()

def draw_ratio(nome_giornali_data_all: list, min_value: int, max_value: int, j, HOURS, DAYS):
    fig, axs = plt.subplots(2, 2, figsize=(25, 15))
    for LINE in range(0, 2):
        for COL in range(0, 2):
            index_hr = LINE * 2 + COL
...
def main():
    ...
    draw_ratio(nome_giornali_local_time, min_value, max_value, 0, 24, DAYS)
    draw_graph_2(nome_giornali_local_time, min_value, max_value, 0, 24, DAYS)
