...
def draw_graph_2(nome_giornali_data_all: list, min_value: int, max_value: int, j, HOURS, couples):
    ...
    plt.savefig(f"BINARY_ANALISI_3HR_{name}_{(j + HOURS) - (HOURS * (no_graph))}_{j + HOURS}.png", dpi=100)
    # plt.show(block=True)
    plt.close()
...
def draw_ratio(nome_giornali_data_all: list, min_value: int, max_value: int, j, HOURS, couples):
    ...
    name = "_".join(couples).replace("/", "_")
    plt.savefig(f"BINARY_RATIO_ANALISI_3HR_{name}_{(j + HOURS) - (HOURS * (no_graph))}_{j + HOURS}.png", dpi=100)
    # plt.show(block=True)
    plt.close()
        ...
        for i in range(0, 24):
            ...
            if (i % (HOURS * 4)) == (HOURS * 4 - HOURS):
                draw_graph_2(nome_giornali_local_time[((i // HOURS) - HOURS): (i // HOURS) + 1], min_value, max_value,
                             i, HOURS, COUPLE_PAPERS)
                draw_ratio(nome_giornali_local_time[((i // HOURS) - HOURS): (i // HOURS) + 1], min_value, max_value,
                             i, HOURS, COUPLE_PAPERS)
