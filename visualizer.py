...
def get_simm_list(directory):
    ...
    for simils in json_file:
        ...
        news_names[simil_format(simils)].append(
            simils["news"][0]["en_title"] + "_____is simil to_____" + simils["news"][1]["en_title"])
                        ...
                        for title_b in to_tab:
                            if title[:title.index("_")] + "--" + title_b[:title_b.index("_")] in simils:
                                with tag('th', klass='table-success', id=title + "--" + title_b):
                                    to_print = ""
                                    for concepts in simil_concepts[
                                        title[:title.index("_")] + "--" + title_b[:title_b.index("_")]]:
                                        to_print += concepts + "--------------"
                                    text(to_print)
                            else:
